...
for t = 1:numSteps
    ...
    switch lower(Param.method)
        case 'isam'
            State.iSAM.t = t;
            iSAMOdometryUpdate_sim(u);
            iSAMMeasurementUpdate(z);
            if mod(t,10) == 0 
                % relinearization
                iSAMRelinearization()
            end
        case 'ekf'
            ...
            ekfpredict_sim(u)
            ekfupdate(z)
            State.Ekf.rPoses = [State.Ekf.rPoses, State.Ekf.mu(1:3)];
        case 'sam'
        case 'ekfisam'
            %% EKF update thing
            tic
            ...
            isam_time = isam_time + toc;
    end
%         State.Ekf.mu_t{t} = State.Ekf.mu;
%         State.Ekf.Sigma_t{t} = State.Ekf.Sigma;
        %=================================================
...
switch lower(Param.method)
    ...
    case 'ekf'
        %% plot robot traj
        plot(State.Ekf.rPoses(1:3:end), State.Ekf.rPoses(2:3:end),'r-');
        %% plot landmark pose
        plot(State.Ekf.mu(4:2:end), State.Ekf.mu(5:2:end),'r*');
    case 'sam'
        
    case 'ekfisam'
        save('sim_ekf_isam_result.mat','State','ekf_time','isam_time','Data')
        %% plot robot traj
        plot(State.iSAM.rlin(1:3:end), State.iSAM.rlin(2:3:end),'r-');
        %% plot landmark pose
        plot(State.iSAM.llin(1:2:end), State.iSAM.llin(2:2:end),'r*');
        ...
        %% plot robot traj
        plot(State.Ekf.rPoses(1:3:end), State.Ekf.rPoses(2:3:end),'c-');
        %% plot landmark pose
        plot(State.Ekf.mu(4:2:end), State.Ekf.mu(5:2:end),'c*');
