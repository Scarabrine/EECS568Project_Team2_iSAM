load('vp_all_pre_data_SAM_result.mat')
...
for iter = 1:1:1000
    ...
    if norm(delta) <= Param.thred_min_delta
        disp('The update delta is too small, exits')
        break
    end
    disp(['current iter: ', num2str(iter), ', delta: ', num2str(norm(delta))])
    toc
...
if iter == Param.MAX_ITER
    disp (['SAM exists at maximum iteration. Last norm of delta: ', num2str(norm(delta))])
end
disp(['SAM exist on iter: ', num2str(iter)])
toc